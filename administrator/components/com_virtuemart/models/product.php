...
class VirtueMartModelProduct extends VmModel
{
  ...
  static function getValidProductFilterArray()
  {
    ...
    if (!isset($filterArray)) {
      ...
      $filterArray = array(
        'product_name', '`p`.created_on', '`p`.product_sku', '`p`.product_mpn',
    ...
    if ($group) {
      ...
      switch ($group) {
        case 'featured':
          ...
          // AstoSoft - start
          $joinPrice = true;
          $orderBy = 'ORDER BY pp.`modified_on` DESC';
          // AstoSoft - end
          break;
        case 'discontinued':
  ...
  public function updateStockInDB($product, $amount, $signInStock, $signOrderedStock)
  {
    ...
    if ($signInStock != '=' or $signOrderedStock != '=') {
      ...
      if ($signInStock == '+') {
        ...
        $db->setQuery(
          'SELECT (IFNULL(`product_in_stock`,"0")+IFNULL(`product_ordered`,"0")) < IFNULL(`low_stock_notification`,"0") '
