...
class VirtueMartModelOrders extends VmModel
{
  ...
  {
    return (bool) (empty($d[$n]) or $d[$n] == 0);
  }

  public static function isNotEmptyDec($d, $n)
  {
    return (bool) (isset($d[$n]) and $d[$n] != 0);
  }
  ...
  function deleteOldPendingOrder($cart)
  {
    ...
    $reUseTimeSql = VmConfig::get('reuseorders', 'PT2M');
    vmdebug('deleteOldPendingOrder ' . $reUseTimeSql);
    ...
    $db->setQuery($q);
    vmdebug('deleteOldPendingOrder', $q);
    $order = $db->loadAssoc();
    if ($order) {
  ...
  private function _writeUserInfo($_id, &$_usr, $_cart)
  {
    ...
    $_userFieldsBT = $_userFieldsModel->getUserFields(
      'account',
    ...
    if ($_cart->ST and empty($_cart->STsameAsBT)) {
      $_userInfoData = array();
      $_userFieldsST = $_userFieldsModel->getUserFields(
        'shipment',
        array('delimiters' => true, 'captcha' => true),
        array('username', 'password', 'password2', 'user_is_vendor')
      );
  ...
  function handleStockAfterStatusChangedPerProduct($newState, $oldState, $tableOrderItems, $quantity)
  {
    ...
    if (!empty($diff) and $product_in_stock == '=' and $product_ordered == '=') {
      ...
      if ($isReserved) {
        if ($diff > 0)  $product_ordered = '+';
        else $product_ordered = '-';
      }
      if ($isOut) {
        if ($diff > 0)  $product_in_stock = '+';
        else $product_ordered = '-';
      }
      $quantity = abs($diff);
    }
  ...
  public function notifyCustomer($virtuemart_order_id, $newOrderData = 0)
  {
    ...
    $vars['url'] = 'url';
    if (!isset($newOrderData['doVendor'])) $vars['doVendor'] = false;
    else $vars['doVendor'] = $newOrderData['doVendor'];
  ...
  public function UpdateOrderHead($virtuemart_order_id, $_orderData)
  {
    ...
    //bill to
    $_userFieldsCart = $_userFieldsModel->getUserFields(
      'account',
      array('delimiters' => true, 'captcha' => true),
      array('username', 'password', 'password2', 'user_is_vendor')
    );
