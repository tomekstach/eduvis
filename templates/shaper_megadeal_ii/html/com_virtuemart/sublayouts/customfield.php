      ...
      switch ($type) {
        ...
        case 'C':
          ...
          foreach ($customfield->selectoptions as $k => $soption) {
            ...
            $attribs = array('class' => 'vm-chzn-select cvselection no-vm-bind sub-variant', 'data-dynamic-update' => '1', 'style' => 'min-width:70px;');
        ...
        case 'M':
          ...
          if (isset($customfield->selectType)) {
            if (empty($customfield->selectType)) {
              ...
              if (!empty($customfield->is_input)) {
                vmJsApi::chosenDropDowns();
                $class = 'class="vm-chzn-select vm-prd-customfields"';
              }
            } else {
          ...
          if ($customfield->is_list and $customfield->is_list != 2) {
            ...
            if (!empty($customfield->is_input)) {
              ...
              $customfields[$selectList[$customfield->virtuemart_custom_id]]->display = JHtml::_(
                $selectType,
        ...
        case 'Z':
          ...
          if ($category = $db->loadObject()) {
            ...
            $customfield->display = JHtml::link(JRoute::_('index.php?option=com_virtuemart&view=category&virtuemart_category_id=' . $category->virtuemart_category_id), $thumb . ' ' . $category->category_name, array('title' => $category->category_name, 'target' => '_self'));
          }
