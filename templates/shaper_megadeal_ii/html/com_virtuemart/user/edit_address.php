 ...
 * @version $Id: edit_address.php 9523 2017-05-04 10:23:55Z Milbo $
 */
...
function renderControlButtons($view,$rview){
	?>
<div class="control-buttons vm-adminform vm-edit-adress-view">
	<?php
	...
	if (VmConfig::get ('oncheckout_show_register', 1) && $view->userDetails->JUser->id == 0 && $view->address_type == 'BT' and $rview == 'cart') {
		?>
		<button name="register" class="btn btn-gray<?php echo $buttonclass ?>" type="submit" onclick="javascript:return myValidator(userForm,true);"
		...
		<?php if (!VmConfig::get ('oncheckout_only_registered', 0)) { ?>
			<button name="save" class="btn btn-gray<?php echo $buttonclass ?>" title="<?php echo vmText::_ ('COM_VIRTUEMART_CHECKOUT_AS_GUEST'); ?>" type="submit"
...
?>
<h1 class="vm-account-details-title"><?php echo $this->page_title ?></h1>
<?php
...
<?php 
	...
	if(VmConfig::get ('reg_captcha') && JFactory::getUser()->guest == 1){
		...
		$hide_captcha = (VmConfig::get ('oncheckout_only_registered') or !$captcha_visible) ? 'style="margin-bottom: 30px;"' : 'style="display: none;"';
		?>
